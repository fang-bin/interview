...
###### 箭头函数的特点：
1. 没有arguments对象（可以使用rest参数代替）
2. 没有prototype对象（意味着其不能作为构造函数，同时也可以通过查验prototype对象来判断是箭头函数还是普通函数）
3. 箭头函数内部并没有this指向，根据作用域的规则，其会直接取其父级（说父级也不准确，应该是作用域链上一级的活动对象）的this对象使用，表现出来的形式：其this指向在定义时确定，不同于普通函数的this指向在调用时确定
4. （由于其内部没有this对象）无法通过apply、bind、call更改this指向（可以通过这些方法更改父级this指向，从而达到修改箭头函数this指向的目的）

###### 不推荐使用箭头函数的情况：
1. 对象方法
2. 原型方法（ES6 Class类定义方法内写的原型方法可以使用）
3. 构造函数（不能作为构造函数）
4. 事件回调方法
5. Vue生命周期和methods方法
